...
KERNEL="linux-cachyos-bore" # linux | linux-lts | linux-zen | linux-hardened | any cachyos kernel
CPU="intel"                 # intel | amd
...
GPU_EXTRA_PACKAGES="both"   # opengl | vulkan | both
...
install_cachyos_repo() {
	...
	run_first_setup() {
		...
		arch_chroot pacman -U "${mirror_cachyos}/cachyos-keyring-20240331-1-any.pkg.tar.zst" \
...
confirm "Do you want to start the installation now?" && install
